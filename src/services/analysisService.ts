...
- MUTLAKA geçerli JSON formatı kullan`;
...
7. SADECE geçerli JSON döndür, başka metin ekleme!`;
...
// 🔧 YENİ: JSON temizleme ve parse fonksiyonu
function cleanAndParseJSON(text: string): any {
  try {
    // 1. Markdown kod bloklarını temizle
    let cleaned = text.replace(/```json\s*/g, '').replace(/```\s*/g, '');
    
    // 2. Başta/sonda boşlukları temizle
    cleaned = cleaned.trim();
    
    // 3. JSON'u bul (en dıştaki {} veya [])
    const jsonMatch = cleaned.match(/(\{[\s\S]*\}|\[[\s\S]*\])/);
    
    if (!jsonMatch) {
      console.error('❌ JSON bulunamadı:', cleaned.substring(0, 200));
      throw new Error('Yanıtta geçerli JSON bulunamadı');
    }
    
    const jsonStr = jsonMatch[0];
    
    // 4. Yaygın JSON hatalarını düzelt
    let fixedJson = jsonStr
      // Virgül eksikliklerini düzelt (en yaygın hata)
      .replace(/"\s*\n\s*"/g, '",\n"')
      // Son virgülleri temizle
      .replace(/,(\s*[}\]])/g, '$1')
      // Çift virgülleri temizle
      .replace(/,,+/g, ',');
    ...
    console.error('Problematik metin:', text.substring(0, 500));
    throw new Error(`JSON parse hatası: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}`);
  }
}
...
export const analysisService = {
  ...
  async detectMatches(base64Image: string): Promise<DetectedMatch[]> {
    try {
      ...
      console.log('📝 OCR Ham yanıt:', content.substring(0, 300));

      // 🔧 YENİ: Gelişmiş JSON parse
      ...
      console.error('❌ detectMatches hatası:', error);
      throw new Error(`Maç tespiti başarısız: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}`);
    }
  },
  ...
  async fetchMatchDataWithGrounding(match: DetectedMatch): Promise<CachedMatchData> {
    try {
      ...
      // 🔧 YENİ: Gelişmiş JSON parse
      const data = cleanAndParseJSON(textContent);
  ...
  async performFinalAnalysis(
    ...
    try {
      ...
      const content = response.data.candidates[0].content.parts[0].text;
      console.log('📝 Final analiz ham yanıt:', content.substring(0, 300));
      ...
      console.error('❌ performFinalAnalysis hatası:', error);
      throw new Error(`Final analiz başarısız: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}`);
    }
  },
