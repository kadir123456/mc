// routes/bulletinAnalysis.js - Bülten Analizi (Kullanıcının Maç Listesi)
const express = require('express');
const axios = require('axios');
const router = express.Router();
const admin = require('firebase-admin');

// ==================== HELPER FUNCTIONS ====================

// Gemini JSON yanıtını güvenli şekilde parse et
function parseGeminiJSON(text) {
  if (!text) {
    throw new Error('Boş yanıt');
  }
  
  try {
    // Markdown kod bloklarını temizle
    let cleanText = text.replace(/```json\s*/g, '').replace(/```\s*/g, '');
    
    // JSON'u bul
    const jsonMatch = cleanText.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      throw new Error('JSON bulunamadı');
    }
    
    // Parse et
    const parsed = JSON.parse(jsonMatch[0]);
    return parsed;
    
  } catch (error) {
    console.error('❌ JSON parse hatası:', error.message);
    console.error('📄 Ham yanıt:', text.substring(0, 500));
    throw new Error(`JSON parse hatası: ${error.message}`);
  }
}

// Takım ismi benzerlik skoru hesapla (Levenshtein Distance + Fuzzy Matching)
function calculateSimilarity(str1, str2) {
  const normalize = (str) => {
    return str
      .toLowerCase()
      .replace(/\s+/g, '')
      .replace(/[.-]/g, '')
      .replace(/ş/g, 's')
      .replace(/ç/g, 'c')
      .replace(/ı/g, 'i')
      .replace(/ğ/g, 'g')
      .replace(/ü/g, 'u')
      .replace(/ö/g, 'o');
  };
  
  const s1 = normalize(str1);
  const s2 = normalize(str2);
  
  // Tam eşleşme
  if (s1 === s2) return 1.0;
  
  // İçerme kontrolü
  if (s1.includes(s2) || s2.includes(s1)) return 0.85;
  
  // Levenshtein distance
  const matrix = Array(s2.length + 1).fill(null).map(() => 
    Array(s1.length + 1).fill(null)
  );
  
  for (let i = 0; i <= s1.length; i++) matrix[0][i] = i;
  for (let j = 0; j <= s2.length; j++) matrix[j][0] = j;
  
  for (let j = 1; j <= s2.length; j++) {
    for (let i = 1; i <= s1.length; i++) {
      const indicator = s1[i - 1] === s2[j - 1] ? 0 : 1;
      matrix[j][i] = Math.min(
        matrix[j][i - 1] + 1,
        matrix[j - 1][i] + 1,
        matrix[j - 1][i - 1] + indicator
      );
    }
  }
  
  const distance = matrix[s2.length][s1.length];
  const maxLength = Math.max(s1.length, s2.length);
  return 1 - (distance / maxLength);
}

// Football API çağrısı (Rate Limit Retry ile)
async function fetchFootballAPIWithRetry(url, params, FOOTBALL_API_KEY, maxRetries = 3) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      const response = await axios.get(url, {
        params,
        headers: {
          'x-rapidapi-key': FOOTBALL_API_KEY,
          'x-rapidapi-host': 'v3.football.api-sports.io'
        },
        timeout: 20000
      });
      return response.data;
    } catch (error) {
      if (error.response?.status === 429 && attempt < maxRetries - 1) {
        const waitTime = 2000 * (attempt + 1); // 2s, 4s, 6s
        console.log(`   ⚠️ Rate limit, ${waitTime/1000}s bekleniyor...`);
        await new Promise(resolve => setTimeout(resolve, waitTime));
        continue;
      }
      throw error;
    }
  }
}

// Detaylı maç istatistiklerini çek (şut, pas, top hakimiyeti, vs.)
async function getDetailedMatchStatistics(fixtureId, teamId, FOOTBALL_API_KEY) {
  try {
    const data = await fetchFootballAPIWithRetry(
      'https://v3.football.api-sports.io/fixtures/statistics',
      { fixture: fixtureId },
      FOOTBALL_API_KEY
    );

    const statistics = data.response || [];
    if (statistics.length === 0) {
      return null;
    }

    // İlgili takımın istatistiklerini bul
    const teamStats = statistics.find(s => s.team.id === teamId);
    if (!teamStats) return null;

    // İstatistikleri obje formatına dönüştür
    const stats = {};
    teamStats.statistics.forEach(stat => {
      const key = stat.type.replace(/\s+/g, '_').toLowerCase();
      let value = stat.value;
      
      // Yüzde değerlerini sayıya çevir
      if (typeof value === 'string' && value.includes('%')) {
        value = parseInt(value.replace('%', ''));
      }
      
      stats[key] = value;
    });

    return {
      shotsTotal: stats.shots_on_goal || stats.total_shots || 0,
      shotsOnTarget: stats.shots_on_goal || 0,
      possession: stats.ball_possession || 0,
      passes: stats.total_passes || 0,
      passAccuracy: stats.passes_% || 0,
      corners: stats.corner_kicks || 0,
      yellowCards: stats.yellow_cards || 0,
      redCards: stats.red_cards || 0,
      fouls: stats.fouls || 0,
      offsides: stats.offsides || 0
    };

  } catch (error) {
    // Detaylı istatistik bulunamadı, null döndür (hata vermeden devam et)
    return null;
  }
}

// Takım istatistikleri çek (Son 10 maç) - GELİŞTİRİLMİŞ VERSİYON
async function getTeamStats(teamId, FOOTBALL_API_KEY) {
  try {
    const data = await fetchFootballAPIWithRetry(
      'https://v3.football.api-sports.io/fixtures',
      { team: teamId, last: 10 },
      FOOTBALL_API_KEY
    );

    const fixtures = data.response || [];
    if (fixtures.length === 0) {
      return null;
    }

    // İstatistikleri hesapla
    let wins = 0, draws = 0, losses = 0;
    let goalsScored = 0, goalsConceded = 0;
    let bttsCount = 0, cleanSheetCount = 0;
    let homeFixtures = [];
    let awayFixtures = [];
    const formArray = [];

    // Detaylı istatistikler için toplamlar
    let totalShots = 0, totalShotsOnTarget = 0;
    let totalPossession = 0, totalPassAccuracy = 0;
    let totalCorners = 0, totalYellowCards = 0;
    let detailedStatsCount = 0;

    for (const fixture of fixtures) {
      const isHome = fixture.teams.home.id === teamId;
      const goalsFor = isHome ? fixture.goals.home : fixture.goals.away;
      const goalsAgainst = isHome ? fixture.goals.away : fixture.goals.home;

      // Form hesapla
      if (goalsFor > goalsAgainst) {
        wins++;
        formArray.push('W');
      } else if (goalsFor < goalsAgainst) {
        losses++;
        formArray.push('L');
      } else {
        draws++;
        formArray.push('D');
      }

      // Goller
      goalsScored += goalsFor || 0;
      goalsConceded += goalsAgainst || 0;

      // BTTS (Both Teams To Score)
      if ((fixture.goals.home || 0) > 0 && (fixture.goals.away || 0) > 0) {
        bttsCount++;
      }

      // Clean Sheet
      if (goalsAgainst === 0) {
        cleanSheetCount++;
      }

      // Ev/Deplasman ayır
      if (isHome) {
        homeFixtures.push(fixture);
      } else {
        awayFixtures.push(fixture);
      }

      // ✅ DETAYLI İSTATİSTİKLERİ ÇEK (Sadece bitmiş maçlar için)
      if (fixture.fixture.status.short === 'FT') {
        try {
          const detailedStats = await getDetailedMatchStatistics(
            fixture.fixture.id, 
            teamId, 
            FOOTBALL_API_KEY
          );

          if (detailedStats) {
            totalShots += detailedStats.shotsTotal || 0;
            totalShotsOnTarget += detailedStats.shotsOnTarget || 0;
            totalPossession += detailedStats.possession || 0;
            totalPassAccuracy += detailedStats.passAccuracy || 0;
            totalCorners += detailedStats.corners || 0;
            totalYellowCards += detailedStats.yellowCards || 0;
            detailedStatsCount++;
          }

          // Rate limit için kısa bekleme
          await new Promise(resolve => setTimeout(resolve, 300));

        } catch (detailError) {
          // Detaylı istatistik hatası - sessizce devam et
          console.log(`   ⚠️ Maç ${fixture.fixture.id} detaylı istatistik atlandı`);
        }
      }
    }

    // Ev/Deplasman formları
    const calculateForm = (fixtures, teamId) => {
      const form = [];
      for (const f of fixtures) {
        const isHome = f.teams.home.id === teamId;
        const goalsFor = isHome ? f.goals.home : f.goals.away;
        const goalsAgainst = isHome ? f.goals.away : f.goals.home;
        if (goalsFor > goalsAgainst) form.push('W');
        else if (goalsFor < goalsAgainst) form.push('L');
        else form.push('D');
      }
      return form.join('');
    };

    // Detaylı istatistik ortalamaları hesapla
    const avgShots = detailedStatsCount > 0 ? (totalShots / detailedStatsCount).toFixed(1) : 0;
    const avgShotsOnTarget = detailedStatsCount > 0 ? (totalShotsOnTarget / detailedStatsCount).toFixed(1) : 0;
    const avgPossession = detailedStatsCount > 0 ? Math.round(totalPossession / detailedStatsCount) : 0;
    const avgPassAccuracy = detailedStatsCount > 0 ? Math.round(totalPassAccuracy / detailedStatsCount) : 0;
    const avgCorners = detailedStatsCount > 0 ? (totalCorners / detailedStatsCount).toFixed(1) : 0;

    return {
      form: formArray.join(''),
      wins,
      draws,
      losses,
      goalsScored,
      goalsConceded,
      goalsAvg: (goalsScored / fixtures.length).toFixed(1),
      concededAvg: (goalsConceded / fixtures.length).toFixed(1),
      homeForm: calculateForm(homeFixtures.slice(0, 5), teamId),
      awayForm: calculateForm(awayFixtures.slice(0, 5), teamId),
      btts: Math.round((bttsCount / fixtures.length) * 100),
      cleanSheet: Math.round((cleanSheetCount / fixtures.length) * 100),
      // ✅ YENİ: Detaylı İstatistikler
      avgShots: avgShots,
      avgShotsOnTarget: avgShotsOnTarget,
      avgPossession: avgPossession,
      avgPassAccuracy: avgPassAccuracy,
      avgCorners: avgCorners,
      detailedStatsAvailable: detailedStatsCount > 0
    };

  } catch (error) {
    console.error(`   ⚠️ Takım ${teamId} istatistikleri alınamadı:`, error.message);
    return null;
  }
}

// Head-to-Head istatistikleri çek
async function getH2H(homeTeamId, awayTeamId, FOOTBALL_API_KEY) {
  try {
    const data = await fetchFootballAPIWithRetry(
      'https://v3.football.api-sports.io/fixtures/headtohead',
      { h2h: `${homeTeamId}-${awayTeamId}`, last: 5 },
      FOOTBALL_API_KEY
    );

    const fixtures = data.response || [];
    if (fixtures.length === 0) {
      return null;
    }

    let homeWins = 0, draws = 0, awayWins = 0, totalGoals = 0;

    for (const fixture of fixtures) {
      const homeGoals = fixture.goals.home || 0;
      const awayGoals = fixture.goals.away || 0;
      totalGoals += homeGoals + awayGoals;

      // Kazananı belirle (ev sahibi perspektifinden)
      const isHomeTeamHome = fixture.teams.home.id === homeTeamId;
      if (homeGoals > awayGoals) {
        if (isHomeTeamHome) homeWins++;
        else awayWins++;
      } else if (homeGoals < awayGoals) {
        if (isHomeTeamHome) awayWins++;
        else homeWins++;
      } else {
        draws++;
      }
    }

    return {
      total: fixtures.length,
      homeWins,
      draws,
      awayWins,
      totalGoals,
      avgGoals: (totalGoals / fixtures.length).toFixed(1)
    };

  } catch (error) {
    console.error(`   ⚠️ H2H alınamadı (${homeTeamId} vs ${awayTeamId}):`, error.message);
    return null;
  }
}

// Gemini API helper fonksiyonu
async function callGeminiAPI(prompt, responseFormat = 'json', temperature = 0.3, GEMINI_API_KEY) {
  if (!GEMINI_API_KEY) {
    throw new Error('Gemini API key bulunamadı');
  }

  const response = await axios.post(
    `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`,
    {
      contents: [{
        parts: [{ text: prompt }]
      }],
      generationConfig: {
        temperature,
        maxOutputTokens: 8000
      }
    },
    {
      headers: { 'Content-Type': 'application/json' },
      timeout: 60000
    }
  );

  return response.data?.candidates?.[0]?.content?.parts?.[0]?.text;
}

// Gemini ile tahmin yap - GELİŞTİRİLMİŞ VERSİYON
async function getGeminiPrediction(match, homeStats, awayStats, h2h, GEMINI_API_KEY) {
  try {
    // Detaylı istatistikleri prompt'a ekle
    const homeDetailedStats = homeStats?.detailedStatsAvailable ? `
- Ortalama Şut: ${homeStats.avgShots}/maç
- Ortalama İsabetli Şut: ${homeStats.avgShotsOnTarget}/maç
- Ortalama Top Hakimiyeti: %${homeStats.avgPossession}
- Ortalama Pas İsabeti: %${homeStats.avgPassAccuracy}
- Ortalama Korner: ${homeStats.avgCorners}/maç` : '- Detaylı istatistik yok';

    const awayDetailedStats = awayStats?.detailedStatsAvailable ? `
- Ortalama Şut: ${awayStats.avgShots}/maç
- Ortalama İsabetli Şut: ${awayStats.avgShotsOnTarget}/maç
- Ortalama Top Hakimiyeti: %${awayStats.avgPossession}
- Ortalama Pas İsabeti: %${awayStats.avgPassAccuracy}
- Ortalama Korner: ${awayStats.avgCorners}/maç` : '- Detaylı istatistik yok';

    const promptText = `Sen profesyonel bir futbol analisti ve istatistik uzmanısın. 
Aşağıdaki GERÇEK VERİLERE DAYALI olarak TAHMİN YAP ve SKORLA.

MAÇ:
Ev Sahibi: ${match.apiMatch.homeTeam}
Deplasman: ${match.apiMatch.awayTeam}
Lig: ${match.apiMatch.league}

==================== EV SAHİBİ İSTATİSTİKLER ====================

TEMEL İSTATİSTİKLER (Son 10 Maç):
- Form: ${homeStats?.form || 'N/A'}
- Galibiyet/Beraberlik/Mağlubiyet: ${homeStats?.wins || 0}/${homeStats?.draws || 0}/${homeStats?.losses || 0}
- Kazanma Oranı: %${homeStats?.wins ? Math.round((homeStats.wins / 10) * 100) : 0}
- Gol Ortalaması: ${homeStats?.goalsAvg || 0} gol/maç
- Gol Yeme: ${homeStats?.concededAvg || 0} gol/maç
- Evdeki Form (Son 5): ${homeStats?.homeForm || 'N/A'}
- BTTS (Karşılıklı Gol): %${homeStats?.btts || 0}
- Clean Sheet: %${homeStats?.cleanSheet || 0}

DETAYLI İSTATİSTİKLER (Son 10 Maç):
${homeDetailedStats}

==================== DEPLASMAN İSTATİSTİKLER ====================

TEMEL İSTATİSTİKLER (Son 10 Maç):
- Form: ${awayStats?.form || 'N/A'}
- Galibiyet/Beraberlik/Mağlubiyet: ${awayStats?.wins || 0}/${awayStats?.draws || 0}/${awayStats?.losses || 0}
- Kazanma Oranı: %${awayStats?.wins ? Math.round((awayStats.wins / 10) * 100) : 0}
- Gol Ortalaması: ${awayStats?.goalsAvg || 0} gol/maç
- Gol Yeme: ${awayStats?.concededAvg || 0} gol/maç
- Deplasmanki Form (Son 5): ${awayStats?.awayForm || 'N/A'}
- BTTS (Karşılıklı Gol): %${awayStats?.btts || 0}
- Clean Sheet: %${awayStats?.cleanSheet || 0}

DETAYLI İSTATİSTİKLER (Son 10 Maç):
${awayDetailedStats}

==================== HEAD-TO-HEAD (Son 5 Karşılaşma) ====================
- Ev Sahibi Galibiyet: ${h2h?.homeWins || 0}
- Beraberlik: ${h2h?.draws || 0}
- Deplasman Galibiyet: ${h2h?.awayWins || 0}
- Ortalama Gol: ${h2h?.avgGoals || 0}

==================== GELİŞMİŞ SKORLAMA SİSTEMİ (Toplam 100 Puan) ====================

Bu verilere dayanarak objektif bir skorlama yap:

1. FORM FARKI (0-30 puan):
   - Kazanma oranlarını karşılaştır (Her %10 fark = 5 puan)
   - Son 5 maçtaki formu değerlendir (W=3, D=1, L=0 puan sistemi)
   - Maksimum 30 puan

2. EV SAHİBİ AVANTAJI (0-15 puan):
   - Ev sahibinin evdeki formu (her W = 3 puan)
   - Deplasman takımının deplasmanki formu (her L = 2 puan ev sahibine)
   - Ev sahipliği genel avantajı: +5 puan
   - Maksimum 15 puan

3. GOL ÜRETİM KABİLİYETİ & HÜCUM GÜCÜ (0-25 puan):
   - Gol ortalaması farkı (her 0.5 gol fark = 3 puan)
   - Şut istatistikleri (daha fazla şut = daha yüksek puan)
   - Top hakimiyeti farkı (Her %10 fark = 2 puan)
   - İsabetli şut farkı (kaliteli hücum göstergesi)
   - Maksimum 25 puan

4. H2H ÜSTÜNLÜĞÜ (0-20 puan):
   - Her galibiyet = 4 puan
   - Beraberlik = 2 puan (her iki takıma)
   - Son karşılaşmalardaki gol trendi
   - Maksimum 20 puan

5. SAVUNMA KALİTESİ (0-10 puan):
   - Clean sheet yüzdesi (Her %10 = 1 puan)
   - Gol yeme ortalaması (az yiyen = yüksek puan)
   - Savunma farkı (her 0.5 gol fark = 2 puan)
   - Maksimum 10 puan

==================== SKORLAMA KURALLARI ====================

- Tüm sayısal verileri dikkate al
- Objektif ol, her kritere matematiksel hesaplama yap
- TOPLAM SKOR = 5 kriterin puanlarının toplamı (0-100)
- Tahmin mantığı:
  * 70-100 puan: Net favori (1 veya 2)
  * 55-69 puan: Hafif favori (1 veya 2 düşük güvenle)
  * 45-54 puan: Dengeli (X veya düşük güvenli tahmin)
  * 0-44 puan: Zayıf veri (düşük güven)

- Confidence hesapla:
  * Form tutarlılığı
  * İstatistik zenginliği
  * H2H veri kalitesi
  * Genel skor seviyesi

JSON formatında yanıt ver:
{
  "prediction": "1" veya "X" veya "2",
  "confidence": 0-100,
  "totalScore": 0-100,
  "scores": {
    "formDifference": 0-30,
    "homeAdvantage": 0-15,
    "goalProduction": 0-25,
    "h2hDominance": 0-20,
    "defenseQuality": 0-10
  },
  "reasoning": "Kısa açıklama (max 200 karakter)",
  "keyFactors": ["faktör1", "faktör2", "faktör3"]
}

KRITIK KURALLAR:
1. SADECE JSON yanıt ver
2. Tüm skorları sayısal verilere dayandır
3. Aynı maç her seferinde tutarlı sonuç vermeli (objektif analiz)
4. Detaylı istatistikler varsa mutlaka kullan
5. Tahmin yaparken ev sahibi avantajını unutma (+10-15%)`;

    const predictionText = await callGeminiAPI(promptText, 'json', 0.1, GEMINI_API_KEY);
    const prediction = parseGeminiJSON(predictionText);
    
    return prediction;

  } catch (error) {
    console.error(`   ⚠️ Gemini tahmini alınamadı:`, error.message);
    return {
      prediction: 'N/A',
      confidence: 0,
      totalScore: 0,
      scores: {
        formDifference: 0,
        homeAdvantage: 0,
        goalProduction: 0,
        h2hDominance: 0,
        defenseQuality: 0
      },
      reasoning: 'Tahmin üretilemedi',
      keyFactors: []
    };
  }
}

    const predictionText = await callGeminiAPI(promptText, 'json', 0.2, GEMINI_API_KEY);
    const prediction = parseGeminiJSON(predictionText);
    
    return prediction;

  } catch (error) {
    console.error(`   ⚠️ Gemini tahmini alınamadı:`, error.message);
    return {
      prediction: 'N/A',
      confidence: 0,
      totalScore: 0,
      scores: {
        formDifference: 0,
        homeAdvantage: 0,
        goalProduction: 0,
        h2hDominance: 0,
        defenseQuality: 0
      },
      reasoning: 'Tahmin üretilemedi',
      keyFactors: []
    };
  }
}

// ==================== BÜLTEN ANALİZ ENDPOINT ====================

router.post('/api/analyze-bulletin-advanced', async (req, res) => {
  try {
    console.log('\n' + '='.repeat(80));
    console.log('📋 BÜLTEN ANALİZİ BAŞLATILIYOR (KULLANICI MAÇ LİSTESİ)');
    console.log('='.repeat(80));

    const { matches, userId, creditsToDeduct } = req.body;
    
    // ENV değişkenleri
    const GEMINI_API_KEY = process.env.EMERGENT_LLM_KEY || process.env.VITE_GEMINI_API_KEY;
    const FOOTBALL_API_KEY = process.env.VITE_FOOTBALL_API_KEY;

    console.log('\n📋 PARAMETRE KONTROLÜ:');
    console.log(`   ├─ userId: ${userId || 'YOK'}`);
    console.log(`   ├─ creditsToDeduct: ${creditsToDeduct || 2}`);
    console.log(`   ├─ Maç sayısı: ${matches?.length || 0}`);
    console.log(`   ├─ GEMINI_API_KEY: ${GEMINI_API_KEY ? '✅ MEVCUT' : '❌ EKSİK'}`);
    console.log(`   └─ FOOTBALL_API_KEY: ${FOOTBALL_API_KEY ? '✅ MEVCUT' : '❌ EKSİK'}`);

    // Validasyonlar
    if (!GEMINI_API_KEY) {
      console.error('\n❌ HATA: Gemini API key bulunamadı');
      return res.status(500).json({ error: 'Gemini API key yapılandırılmamış' });
    }

    if (!FOOTBALL_API_KEY) {
      console.error('\n❌ HATA: Football API key bulunamadı');
      return res.status(500).json({ error: 'Football API key yapılandırılmamış' });
    }

    if (!matches || !Array.isArray(matches) || matches.length === 0) {
      console.error('\n❌ HATA: Maç listesi eksik veya geçersiz');
      return res.status(400).json({ 
        error: 'Maç listesi bulunamadı',
        details: 'Lütfen en az bir maç girin'
      });
    }

    // ========== KREDİ KONTROLÜ VE DÜŞÜRME ==========
    let userData = null;
    let initialCredits = 0;

    if (userId && creditsToDeduct) {
      try {
        console.log('\n💰 KREDİ KONTROLÜ:');
        const userRef = admin.database().ref(`users/${userId}`);
        const userSnapshot = await userRef.once('value');
        userData = userSnapshot.val();

        if (!userData) {
          console.error(`   ❌ Kullanıcı bulunamadı: ${userId}`);
          return res.status(404).json({ error: 'Kullanıcı bulunamadı' });
        }

        initialCredits = userData.credits || 0;
        console.log(`   ├─ Mevcut kredi: ${initialCredits}`);
        console.log(`   └─ Gerekli kredi: ${creditsToDeduct}`);

        if (initialCredits < creditsToDeduct) {
          console.error(`   ❌ Yetersiz kredi!`);
          return res.status(403).json({ 
            error: 'Yetersiz kredi',
            currentCredits: initialCredits,
            requiredCredits: creditsToDeduct
          });
        }

      } catch (firebaseError) {
        console.error('\n❌ Firebase hatası:', firebaseError.message);
        return res.status(500).json({ error: 'Kullanıcı verisi kontrol edilemedi' });
      }
    }

    console.log(`\n📊 Kullanıcıdan gelen maçlar:`);
    matches.forEach((m, i) => {
      console.log(`   ${i + 1}. ${m.homeTeam} vs ${m.awayTeam}`);
    });

    // ========== ADIM 1: Football API'den Yaklaşan Maçları Al ==========
    console.log('\n' + '─'.repeat(80));
    console.log('⚽ ADIM 1: FOOTBALL API\'DEN YAKLAŞAN MAÇLARI ÇEKME');
    console.log('─'.repeat(80));
    
    const today = new Date();
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    
    const todayStr = today.toISOString().split('T')[0];
    const tomorrowStr = tomorrow.toISOString().split('T')[0];
    
    console.log(`   ├─ Bugün: ${todayStr}`);
    console.log(`   └─ Yarın: ${tomorrowStr}`);
    
    // İki istek paralel olarak
    const [todayResponse, tomorrowResponse] = await Promise.all([
      axios.get('https://v3.football.api-sports.io/fixtures', {
        params: { date: todayStr },
        headers: {
          'x-rapidapi-host': 'v3.football.api-sports.io',
          'x-rapidapi-key': FOOTBALL_API_KEY
        },
        timeout: 20000
      }),
      axios.get('https://v3.football.api-sports.io/fixtures', {
        params: { date: tomorrowStr },
        headers: {
          'x-rapidapi-host': 'v3.football.api-sports.io',
          'x-rapidapi-key': FOOTBALL_API_KEY
        },
        timeout: 20000
      })
    ]);
    
    // Maçları birleştir
    const allFixtures = [
      ...(todayResponse.data.response || []),
      ...(tomorrowResponse.data.response || [])
    ];
    
    console.log(`\n✅ Football API yanıtı alındı:`);
    console.log(`   ├─ Bugün: ${todayResponse.data.response?.length || 0} maç`);
    console.log(`   ├─ Yarın: ${tomorrowResponse.data.response?.length || 0} maç`);
    console.log(`   └─ Toplam: ${allFixtures.length} maç`);

    if (allFixtures.length === 0) {
      console.error('\n❌ HATA: Football API\'den maç alınamadı');
      return res.json({
        success: false,
        message: 'Football API\'den maç bilgisi alınamadı.',
        matches: [],
        creditsDeducted: 0,
        remainingCredits: initialCredits
      });
    }

    // ========== ADIM 2: FUZZY MATCHING İLE MAÇ EŞLEŞTİRME ==========
    console.log('\n' + '─'.repeat(80));
    console.log('🔗 ADIM 2: FUZZY MATCHING İLE MAÇ EŞLEŞTİRME');
    console.log('─'.repeat(80));
    
    const matchedMatches = [];
    const unmatchedMatches = [];

    for (let idx = 0; idx < matches.length; idx++) {
      const userMatch = matches[idx];
      const homeSearch = userMatch.homeTeam;
      const awaySearch = userMatch.awayTeam;
      
      console.log(`\n🔍 Maç ${idx + 1}/${matches.length}: ${homeSearch} vs ${awaySearch}`);

      // Fuzzy matching ile en iyi eşleşmeyi bul
      let bestMatch = null;
      let bestScore = 0;

      for (const fixture of allFixtures) {
        const apiHome = fixture.teams.home.name;
        const apiAway = fixture.teams.away.name;
        
        // Her iki takım için benzerlik skorları hesapla
        const homeSimilarity = calculateSimilarity(apiHome, homeSearch);
        const awaySimilarity = calculateSimilarity(apiAway, awaySearch);
        
        // Ortalama skor
        const avgScore = (homeSimilarity + awaySimilarity) / 2;
        
        // En az %75 benzerlik gerekli
        if (avgScore >= 0.75 && avgScore > bestScore) {
          bestScore = avgScore;
          bestMatch = {
            fixture,
            homeScore: homeSimilarity,
            awayScore: awaySimilarity,
            avgScore
          };
        }
      }

      if (bestMatch) {
        console.log(`   ├─ ✅ Eşleşme Bulundu:`);
        console.log(`   │  └─ ${bestMatch.fixture.teams.home.name} vs ${bestMatch.fixture.teams.away.name}`);
        console.log(`   ├─ 📈 Benzerlik Skorları:`);
        console.log(`   │  ├─ Ev Sahibi: %${(bestMatch.homeScore * 100).toFixed(0)}`);
        console.log(`   │  ├─ Deplasman: %${(bestMatch.awayScore * 100).toFixed(0)}`);
        console.log(`   │  └─ Ortalama: %${(bestMatch.avgScore * 100).toFixed(0)}`);
        
        matchedMatches.push({
          extracted: {
            homeTeam: userMatch.homeTeam,
            awayTeam: userMatch.awayTeam
          },
          apiMatch: {
            fixtureId: bestMatch.fixture.fixture.id,
            homeTeam: bestMatch.fixture.teams.home.name,
            awayTeam: bestMatch.fixture.teams.away.name,
            league: bestMatch.fixture.league.name,
            date: bestMatch.fixture.fixture.date,
            status: bestMatch.fixture.fixture.status.short,
            homeTeamId: bestMatch.fixture.teams.home.id,
            awayTeamId: bestMatch.fixture.teams.away.id
          },
          matchScore: Math.round(bestMatch.avgScore * 100)
        });
        console.log(`   └─ ✅ Eşleşme kabul edildi\n`);
      } else {
        unmatchedMatches.push(userMatch);
        console.log(`   └─ ❌ Eşleşme bulunamadı (%75 altında)\n`);
      }
      
      // Rate limit için kısa bekleme
      await new Promise(resolve => setTimeout(resolve, 200));
    }

    // ========== ADIM 3: KAPSAMLI İSTATİSTİKLER ÇEK ==========
    console.log('\n' + '─'.repeat(80));
    console.log('📊 ADIM 3: KAPSAMLI İSTATİSTİKLER ÇEKİLİYOR');
    console.log('─'.repeat(80));

    for (let idx = 0; idx < matchedMatches.length; idx++) {
      const match = matchedMatches[idx];
      console.log(`\n⚽ Maç ${idx + 1}/${matchedMatches.length}: ${match.apiMatch.homeTeam} vs ${match.apiMatch.awayTeam}`);
      
      try {
        const homeTeamId = match.apiMatch.homeTeamId;
        const awayTeamId = match.apiMatch.awayTeamId;

        // Ev sahibi istatistikleri
        console.log(`   ├─ 📈 Ev sahibi son 10 maç alınıyor...`);
        const homeStats = await getTeamStats(homeTeamId, FOOTBALL_API_KEY);
        if (homeStats) {
          console.log(`   │  ├─ Form: ${homeStats.form} (${homeStats.wins}G ${homeStats.draws}B ${homeStats.losses}M)`);
          console.log(`   │  ├─ Gol Ort: ${homeStats.goalsAvg} | Yenen: ${homeStats.concededAvg}`);
          if (homeStats.detailedStatsAvailable) {
            console.log(`   │  ├─ ✅ Detaylı İstatistikler Mevcut`);
            console.log(`   │  │  ├─ Şut: ${homeStats.avgShots} | İsabetli: ${homeStats.avgShotsOnTarget}`);
            console.log(`   │  │  ├─ Top Hakimiyeti: %${homeStats.avgPossession} | Pas İsabeti: %${homeStats.avgPassAccuracy}`);
            console.log(`   │  │  └─ Korner: ${homeStats.avgCorners}`);
          } else {
            console.log(`   │  └─ ⚠️ Detaylı istatistik yok (sadece temel veriler)`);
          }
        } else {
          console.log(`   │  └─ ⚠️ İstatistik alınamadı`);
        }

        // Deplasman istatistikleri
        console.log(`   ├─ 📉 Deplasman son 10 maç alınıyor...`);
        const awayStats = await getTeamStats(awayTeamId, FOOTBALL_API_KEY);
        if (awayStats) {
          console.log(`   │  ├─ Form: ${awayStats.form} (${awayStats.wins}G ${awayStats.draws}B ${awayStats.losses}M)`);
          console.log(`   │  ├─ Gol Ort: ${awayStats.goalsAvg} | Yenen: ${awayStats.concededAvg}`);
          if (awayStats.detailedStatsAvailable) {
            console.log(`   │  ├─ ✅ Detaylı İstatistikler Mevcut`);
            console.log(`   │  │  ├─ Şut: ${awayStats.avgShots} | İsabetli: ${awayStats.avgShotsOnTarget}`);
            console.log(`   │  │  ├─ Top Hakimiyeti: %${awayStats.avgPossession} | Pas İsabeti: %${awayStats.avgPassAccuracy}`);
            console.log(`   │  │  └─ Korner: ${awayStats.avgCorners}`);
          } else {
            console.log(`   │  └─ ⚠️ Detaylı istatistik yok (sadece temel veriler)`);
          }
        } else {
          console.log(`   │  └─ ⚠️ İstatistik alınamadı`);
        }

        // Head-to-Head
        console.log(`   ├─ 🔄 H2H son 5 karşılaşma alınıyor...`);
        const h2h = await getH2H(homeTeamId, awayTeamId, FOOTBALL_API_KEY);
        if (h2h) {
          console.log(`   │  └─ ${h2h.homeWins}-${h2h.draws}-${h2h.awayWins} (Ort. ${h2h.avgGoals} gol)`);
        } else {
          console.log(`   │  └─ ⚠️ H2H verisi yok`);
        }

        // İstatistikleri match objesine ekle
        match.statistics = {
          homeTeam: homeStats,
          awayTeam: awayStats,
          h2h: h2h
        };

        console.log(`   └─ ✅ İstatistikler tamamlandı`);

        // Rate limit için bekleme
        await new Promise(resolve => setTimeout(resolve, 500));

      } catch (error) {
        console.log(`   └─ ❌ Hata: ${error.message} - İstatistik atlanıyor`);
        // Hata durumunda boş istatistikler ekle
        match.statistics = {
          homeTeam: null,
          awayTeam: null,
          h2h: null
        };
      }
    }

    // ========== ADIM 4: GEMİNİ İLE KAPSAMLI TAHMİNLER ==========
    console.log('\n' + '─'.repeat(80));
    console.log('🤖 ADIM 4: GEMİNİ İLE KAPSAMLI TAHMİNLER');
    console.log('─'.repeat(80));

    const BATCH_SIZE = 3;
    for (let i = 0; i < matchedMatches.length; i += BATCH_SIZE) {
      const batch = matchedMatches.slice(i, i + BATCH_SIZE);
      
      console.log(`\n📦 Batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(matchedMatches.length / BATCH_SIZE)}: ${batch.length} maç`);

      await Promise.all(batch.map(async (match, batchIdx) => {
        const globalIdx = i + batchIdx;
        console.log(`\n📊 Maç ${globalIdx + 1}/${matchedMatches.length}: ${match.apiMatch.homeTeam} vs ${match.apiMatch.awayTeam}`);
        
        try {
          if (!match.statistics || (!match.statistics.homeTeam && !match.statistics.awayTeam)) {
            console.log(`   └─ ⚠️ İstatistik yok, tahmin atlanıyor`);
            return;
          }

          console.log(`   ├─ 🤖 Gemini tahmini üretiliyor...`);
          const prediction = await getGeminiPrediction(
            match,
            match.statistics.homeTeam,
            match.statistics.awayTeam,
            match.statistics.h2h,
            GEMINI_API_KEY
          );

          // Tahmin verilerini match objesine ekle
          match.prediction = prediction.prediction;
          match.confidence = prediction.confidence;
          match.totalScore = prediction.totalScore;
          match.scores = prediction.scores;
          match.reasoning = prediction.reasoning;
          match.keyFactors = prediction.keyFactors;

          console.log(`   ├─ Toplam Skor: ${match.totalScore}/100`);
          console.log(`   │  ├─ Form Farkı: ${match.scores.formDifference}/30`);
          console.log(`   │  ├─ Ev Avantajı: ${match.scores.homeAdvantage}/15`);
          console.log(`   │  ├─ Gol Üretimi: ${match.scores.goalProduction}/25`);
          console.log(`   │  ├─ H2H: ${match.scores.h2hDominance}/20`);
          console.log(`   │  └─ Savunma: ${match.scores.defenseQuality}/10`);
          console.log(`   ├─ 🎯 Tahmin: ${match.prediction} (Güven: %${match.confidence})`);
          console.log(`   └─ 💡 ${match.reasoning}`);

        } catch (error) {
          console.log(`   └─ ❌ Tahmin hatası: ${error.message}`);
          // Hata durumunda boş tahmin ekle
          match.prediction = 'N/A';
          match.confidence = 0;
          match.totalScore = 0;
          match.scores = {
            formDifference: 0,
            homeAdvantage: 0,
            goalProduction: 0,
            h2hDominance: 0,
            defenseQuality: 0
          };
          match.reasoning = 'Tahmin üretilemedi';
          match.keyFactors = [];
        }
      }));

      // Batch'ler arası 1 saniye bekle
      if (i + BATCH_SIZE < matchedMatches.length) {
        console.log(`\n⏳ Sonraki batch için 1 saniye bekleniyor...`);
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    }

    // ========== KREDİ DÜŞÜRME (BAŞARILI ANALİZ SONRASI) ==========
    let remainingCredits = initialCredits;

    if (userId && creditsToDeduct && userData && matchedMatches.length > 0) {
      try {
        console.log('\n💰 KREDİ DÜŞÜRME:');
        const userRef = admin.database().ref(`users/${userId}`);
        const newCredits = initialCredits - creditsToDeduct;
        
        await userRef.update({
          credits: newCredits
        });
        
        remainingCredits = newCredits;
        console.log(`   ├─ Düşürülen: ${creditsToDeduct} kredi`);
        console.log(`   └─ Kalan: ${remainingCredits} kredi`);
        
        // Transaction kaydı oluştur
        const transactionRef = admin.database().ref(`users/${userId}/transactions`).push();
        await transactionRef.set({
          type: 'bulletin_analysis',
          credits: -creditsToDeduct,
          matchCount: matchedMatches.length,
          timestamp: Date.now(),
          date: new Date().toISOString()
        });
        
        console.log(`   └─ ✅ Transaction kaydedildi`);
        
      } catch (creditError) {
        console.error('\n❌ Kredi düşürme hatası:', creditError.message);
        // Hata olsa bile sonuçları döndür
      }
    }

    // ========== SONUÇ ==========
    console.log('\n' + '='.repeat(80));
    console.log('✅ BÜLTEN ANALİZİ TAMAMLANDI!');
    console.log('='.repeat(80));
    console.log(`\n📊 ÖZET:`);
    console.log(`   ├─ Kullanıcıdan gelen maç: ${matches.length}`);
    console.log(`   ├─ Başarıyla eşleştirilen: ${matchedMatches.length}`);
    console.log(`   ├─ İstatistik çekilen: ${matchedMatches.filter(m => m.statistics?.homeTeam).length}`);
    console.log(`   ├─ Tahmin üretilen: ${matchedMatches.filter(m => m.prediction && m.prediction !== 'N/A').length}`);
    console.log(`   ├─ Eşleşmeyen: ${unmatchedMatches.length}`);
    console.log(`   ├─ Kredi düşürüldü: ${creditsToDeduct || 0}`);
    console.log(`   └─ Kalan kredi: ${remainingCredits}`);
    console.log('\n' + '='.repeat(80) + '\n');

    res.json({
      success: true,
      message: `${matchedMatches.length} maç başarıyla analiz edildi`,
      creditsDeducted: creditsToDeduct || 0,
      remainingCredits: remainingCredits,
      matches: matchedMatches,
      unmatchedMatches: unmatchedMatches
    });

  } catch (error) {
    console.error('\n' + '='.repeat(80));
    console.error('❌ BÜLTEN ANALİZ HATASI');
    console.error('='.repeat(80));
    console.error(`Hata: ${error.message}`);
    console.error(`Stack: ${error.stack}`);
    console.error('='.repeat(80) + '\n');
    
    const errorMessage = error.message || 'Bilinmeyen hata';
    const errorDetails = error.response?.data?.error?.message || errorMessage;
    
    res.status(500).json({ 
      error: 'Bülten analizi yapılamadı',
      details: errorDetails,
      message: 'Lütfen tekrar deneyin'
    });
  }
});

module.exports = router;
